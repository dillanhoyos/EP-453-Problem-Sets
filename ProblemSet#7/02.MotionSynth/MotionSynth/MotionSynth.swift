...
class MotionSynth {
    ...
    init(){
        ...
        flanger = Flanger(fmOscillator, frequency: 1000, depth: 0.9, feedback: 0.2, dryWetMix: 0.5)
        
        filter = MoogLadder(flanger!, cutoffFrequency: 5000, resonance: 0.3)
    ...
    func setFLanger(accelX: Double, accelY: Double){
        let frequency = 1000 * scale(num: CGFloat(accelY), minNum: -1.5, maxNum: 1.5, scaleMin: 0, scaleMax: 1)
        
        let depth = 0.33 * scale(num: CGFloat(accelX), minNum: -1.5, maxNum: 1.5, scaleMin: 0, scaleMax: 1)
        
        flanger!.frequency = Float(1000 + frequency)
        flanger!.depth = Float(0.6 + depth)
