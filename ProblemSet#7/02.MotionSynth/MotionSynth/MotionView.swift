...
struct NewV: View {
    let screenSize = UIScreen.main.bounds
    var positio:CGPoint
    var rotationAngl:CGFloat
    
    var body: some View{
        VStack{
            Circle()
                .fill(Color.init(hue: 1, saturation: 1, brightness: 1, opacity:
                                    0.6))
                .frame(width: 20, height:20, alignment: .bottomTrailing)
                .transformEffect(
                    CGAffineTransform(rotationAngle: rotationAngl)
                       .translatedBy(x: self.positio.x, y: self.positio.y)
                     )
                .frame(width:screenSize.width, height:screenSize.height)
