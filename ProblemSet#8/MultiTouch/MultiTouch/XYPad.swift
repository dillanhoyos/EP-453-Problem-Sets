...
class MultiTouchController2: UIViewController {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        for touch in touches {
            ...
            if fingers[touch] != nil {
                var location = touch.location(in: self.view)
              //finger.update(id: touch.hash, point: location
                location.x = location.x/self.view.frame.size.width
                location.y = location.y/self.view.frame.size.height
                //Store touch positions in a dictionary
...
struct XYPadView: UIViewControllerRepresentable {
    typealias UIViewControllerType = MultiTouchController2
    
    func makeUIViewController(context: Context) -> MultiTouchController2 {
        let controller = MultiTouchController2()
        return controller
    }
    
    func updateUIViewController(_ uiViewController: MultiTouchController2, context: Context) {}
}
